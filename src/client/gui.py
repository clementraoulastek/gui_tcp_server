...
from src.tools.utils import Color, Icon, get_scaled_icon


class Gui:
    def __init__(self, title: str, root: None = None) -> None:
        ...
        self.scrolled_text = customtkinter.CTkTextbox(
            main_frame, state="disabled", height=450
        )
        ...
        if message := self.entry.get():
            if self.update_login:
                self._diplay_message_and_clear(
                    "Login updated: ", message, is_from_server=True
                )
    ...
    def _diplay_message_and_clear(
        ...
        if id_sender != "Date: ":
            self.scrolled_text.insert(tkinter.END, f"{id_sender}", "id")
    ...
    def _display_message(self, message: str, is_from_server, doubleReturn=False):
        ...
        if ":" in message and "Date" not in message:
            ...
            self.scrolled_text.insert(tkinter.END, f"{id}:", "id")
        self.scrolled_text.insert(
            tkinter.END,
    ...
    def config(self):
        ...
        self.scrolled_text.insert(tkinter.END, config, "server")
        self.scrolled_text.configure(state="disabled")
